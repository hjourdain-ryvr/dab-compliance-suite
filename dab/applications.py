...
def launch(test_result, durationInMs=0,expectedLatencyMs=0):
    ...
    sleep(5)
    return YesNoQuestion(test_result, "YouTube started?") and Default_Validations(test_result, durationInMs, expectedLatencyMs)

def launch_with_content(test_result, durationInMs=0,expectedLatencyMs=0):
    dab_response_validator.validate_dab_response_schema(test_result.response)
    sleep(5)
    return YesNoQuestion(test_result, "YouTube started with playback?") and Default_Validations(test_result, durationInMs, expectedLatencyMs)
...
def exit(test_result, durationInMs=0,expectedLatencyMs=0):
    ...
    sleep(5)
    return YesNoQuestion(test_result, "YouTube exited?") and Default_Validations(test_result, durationInMs, expectedLatencyMs)
